...
public class VersionHelperController extends AppBaseController implements Initializable {
    ...
    public void commitRemind(ActionEvent actionEvent) throws GitLabApiException, ParseException {
        try {
            //判断数据是否存在
            if (GitCommitCacheDate.commitInfos.isEmpty() && GitCommitCacheDate.commitSqlFile.isEmpty()) {
    ...
    public void setCommitInfo(GitLabApi gitLabApi) throws GitLabApiException, ParseException {
        ...
        for (GitlabProjectConfig gitlabProjectConfig : gitlabProjectConfigs) {
            ...
            if ("sql-script".equals(gitlabProjectConfig.getProjectName())) {
                for (Commit commit : commits) {
                    if (gitAccount.getText().equals(commit.getCommitterName())) {
                        ...
                        for (Diff diff : diffs) {
                            if (!GitCommitCacheDate.commitSqlFile.contains(diff.getOldPath())) {
                                GitCommitCacheDate.commitSqlFile.add(diff.getOldPath());
                            }
                            if (!GitCommitCacheDate.commitSqlFile.contains(diff.getNewPath())) {
                                GitCommitCacheDate.commitSqlFile.add(diff.getNewPath());
                            }
                        }
                    }
                }
            ...
            if (!commitInfo.getCommitFiles().isEmpty()) {
                GitCommitCacheDate.commitInfos.add(commitInfo);
            }
        }
    }
    ...
    public void createMrRequest(ActionEvent actionEvent) throws GitLabApiException {
        ...
        //如果数据为空不做处理
        if (GitCommitCacheDate.commitInfos.isEmpty()) {
            tip.setText("数据为空");
            return;
        }
        ArrayList<String> messages = new ArrayList<>();
        for (CommitInfo commitInfo : GitCommitCacheDate.commitInfos) {
            //提交合并请求
    ...
    public void loadCommitInfo(ActionEvent actionEvent) {
        try {
            ...
            //清除数据
            GitCommitCacheDate.commitSqlFile.clear();
            GitCommitCacheDate.commitInfos.clear();
            //获取提交信息
            setCommitInfo(gitLabApi);
            tip.setText("加载数据成功");
            if (GitCommitCacheDate.dataIsEmpty()) {
                tip.setText("加载的数据为空");
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void showCommitInfo(ActionEvent actionEvent) throws IOException {
        ...
        try {
            ...
            if (!GitCommitCacheDate.commitInfos.isEmpty() || !GitCommitCacheDate.commitSqlFile.isEmpty()) {
                ...
                fileWriter.write("\n");
                fileWriter.write("\n");
                fileWriter.write("脚本涉及文件变动数为：" + GitCommitCacheDate.commitSqlFile.size());
                for (String s : GitCommitCacheDate.commitSqlFile) {
                    fileWriter.write("\n");
                    fileWriter.write("  " + s);
                }
            ...
            fileWriter.write(RemineController.getNewDescript());
            fileWriter.flush();
